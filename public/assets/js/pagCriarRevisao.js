...
let selected_category_title;

console.log("JavaScript revisao.js foi aberto!\n");

document.getElementById('generateButton').addEventListener("click", async (event)=>{
        event.preventDefault();
        startLoader();
        var select = document.getElementById('selectedCategory');
        var selected_category_title = select.options[select.selectedIndex].text;
        selected_category_id = document.getElementById("selectedCategory").value;

        review_title = document.getElementById("reviewTitle").value;
        prompt = selected_category_title + " - " + review_title;
    ...
    if(response_result == 1){
        ...
        $(document).ready(function() {
            ...
            .done(function(response) {
                ...
                endLoader();
                window.location.href = 'Revisar.php';
            })
